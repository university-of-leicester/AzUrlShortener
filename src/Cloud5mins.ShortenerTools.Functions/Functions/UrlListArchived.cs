...
{
    public class UrlListArchived
    {

        private readonly ILogger _logger;
        private readonly ShortenerSettings _settings;

        public UrlListArchived(ILoggerFactory loggerFactory, ShortenerSettings settings)
        {
            _logger = loggerFactory.CreateLogger<UrlList>();
            _settings = settings;
        }

        [Function("UrlListArchived")]
        public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "api/UrlListArchived")] HttpRequestData req, ExecutionContext context)
        {
            ...
            try
            {
                ...
                result.UrlList = result.UrlList.Where(p => (p.IsArchived == true)).ToList();
